...
local function LoadMod(ModName, ModInfo, World)
    ...
    if UnrealVersion.IsBelow(5, 1) then
        ModPath[1] = string.format("%s.%s", ModInfo.AssetPath, ModInfo.AssetName)
        Log(string.format("Path: %s.%s\n", ModInfo.AssetPath, ModInfo.AssetName))
    else
        AssetData = {
    ...
    ExecuteInGameThread(function()
        local Assets = GSE_AssetUtilFuncLib:GetAssetsByPath(ModPath)
        
        if not Assets:IsValid() then
            Log(string.format("Assets Not Valid\n"))
            return
        end
        -- GSE_AssetUtilFuncLib:LoadObjectsFromAssetData(Assets)
        local ModClass = StaticFindObject(ModPath[1])
...
local function CacheGSE_AssetUtilFuncLib()
    if GSE_AssetUtilFuncLib then return end

    GSE_AssetUtilFuncLib = StaticFindObject("/Script/UnrealExtent.Default__GSE_AssetUtilFuncLib")
    if GSE_AssetUtilFuncLib:IsValid() then return end

    error("GSE_AssetUtilFuncLib is not valid\n")
end



local function LoadMods(World)
    CacheGSE_AssetUtilFuncLib()
...
RegisterLoadMapPostHook(function(Engine, World)
    if MapsLoaded == 1 then
        LoadMods(World:get())
    end
    if MapsLoaded == 0 then
